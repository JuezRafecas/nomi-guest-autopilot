...
export type SegmentRowActionMode = 'cta' | 'coming-soon';
...
export function SegmentRow({
  ...
  const wrapperClass = interactive
    ? 'group relative block transition-colors duration-200 hover:bg-bg-sunken'
    : 'relative block';
  const wrapperStyle = { borderBottom: '1px solid var(--hairline)' } as const;

  const accentClass = interactive
    ? 'absolute left-0 top-0 bottom-0 transition-[width] duration-200 group-hover:w-[6px] w-[3px]'
  ...
  const content = (
    <>
      ...
      <div className="grid grid-cols-[minmax(0,1fr)_110px_80px_180px_auto] items-center gap-8 pl-8 pr-6 py-7">
        ...
        <div className="text-right">
          {actionMode === 'coming-soon' ? (
            <span
              className="inline-flex items-center text-[11px] uppercase font-[600] px-3 py-1.5"
              style={{
