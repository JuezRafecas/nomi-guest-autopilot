    ...
    <AppShell>
      ...
      <section className="editorial-container section-pt-lead section-pb-close">
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 md:gap-10">
          ...
          >
            <span className="italic">{totalGuests}</span> comensales en la base.
          </h1>
          <div className="md:text-right shrink-0 md:pb-4 flex items-baseline gap-3 md:block">
            <div className="text-[10px] uppercase tracking-label text-fg-subtle">Base total</div>
            <div className="font-mono text-2xl md:text-3xl text-fg-muted tabular-nums md:mt-1">
              <Numeral value={totalGuests} /> contactos
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="editorial-container pb-4">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3 mb-3">
          <Label>Comensales destacados</Label>
          <span
            className="font-mono text-[10.5px] uppercase tabular-nums"
            style={{ letterSpacing: '0.14em', color: 'var(--fg-subtle)' }}
          >
            Mostrando <span style={{ color: 'var(--fg)' }}>{filtered.length}</span> / {guests.length}
          </span>
        </div>
        <div className="border-t border-hairline pt-3">
          <FilterBar
            label="Filtrar comensales por segmento"
      ...
      <section className="editorial-container pb-24">
        {filtered.length === 0 ? (
          <div className="border-t border-hairline">
            <EmptyState
              title="Nadie en este segmento."
              hint="Probá con otro segmento o esperá a que entren nuevos comensales en la base."
            />
          </div>
        ) : (
